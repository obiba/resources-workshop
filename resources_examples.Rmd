...
3. Using `dsBase` functions:
     - Check that the number of observations at each study (variable `Type`) is 42.
     - Create a boxplot of the CO2 uptake (variable `uptake`) by study.
     - Create an scatterplot of the CO2 uptake (X-axis) vs CO2 concentration (variable `conc`) (Y-axix).
     - Run a linear model to investigate whether there are differences between the two studies (`Type`) adjusting for chilled overnight factor (`Treatment`).
