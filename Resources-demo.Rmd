...
Load the [resourcer](https://cran.r-project.org/package=R6) package (see also [source repository](https://github.com/obiba/resourcer). The output of this library load informs about the available resource **resolvers**. These are responsible for building the appropriate connector to a resource object. This set of resolvers can be extended to new types of resources.
...
```{r CAPostalCodes.data}
CAPostalCodes.data <- CAPostalCodes.client$asDataFrame()
knitr::kable(head(CAPostalCodes.data))
```
...
knitr::kable(head(gps_participant.data))
```
...
* `output` the character vector of the connad output,
